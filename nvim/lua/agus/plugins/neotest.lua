...
return {
    ...
    dependencies = {
        ...
        -- Neotest adapters
        "mrcjkb/rustaceanvim", "fredrikaverpil/neotest-golang"
    },
    -- We cannot use opts because we need to load the adapter
    config = function()
        require("neotest").setup({
            adapters = {
                require("rustaceanvim.neotest"), require("neotest-golang")
            },
            summary = {mappings = {target = "T"}}
        })
    end,
    keys = {
        {"tS", function() require("neotest").run.stop() end, desc = "Stop test"},
